          ...
          <Stack direction="row" spacing={1.5}>
            <FormControl size="small" sx={{ minWidth: 130 }}>
              ...
              <Select label="Year" value={String(year)} onChange={event => setYear(Number(event.target.value))}>
                {years.map(option => <MenuItem key={option} value={String(option)}>{option}</MenuItem>)}
              </Select>
            </FormControl>
...
const PivotSection = ({ title, kind, records, mode }: { title: string; kind: RecordKind; records: FinanceRecord[]; mode: ModeFilter }) => {
  ...
  return (
    <Card variant="outlined" sx={{ borderRadius: 3, overflow: "hidden" }}>
      <Stack direction="row" justifyContent="space-between" sx={{ px: 2.5, py: 2, bgcolor: softColor, borderBottom: "1px solid", borderColor: "divider" }}>
        <Typography variant="h6" sx={{ color, fontWeight: 700 }}>{title}</Typography>
        <Typography variant="caption" color="text.secondary">{data.categories.length} categories</Typography>
      </Stack>
        ...
        <Table size="small">
          <TableHead>
            <TableRow>
              ...
              {monthLabels.map(label => <TableCell key={label} align="right">{label.slice(0, 3)}</TableCell>)}
              ...
              data.categories.flatMap(category => modes.map((currentMode, index) => {
                ...
                return (
                  <TableRow key={`${category}-${currentMode}`}>
                    {index === 0 && <TableCell rowSpan={modes.length} sx={{ fontWeight: 700 }}>{category}</TableCell>}
                    {mode === "both" && <TableCell><Typography variant="caption" sx={{ textTransform: "uppercase", letterSpacing: "0.12em", color: currentMode === "planned" ? "#5a75bd" : "text.primary", fontWeight: currentMode === "tracked" ? 700 : 500 }}>{currentMode}</Typography></TableCell>}
                    {values.map((cell, monthIndex) => <TableCell key={monthIndex} align="right" sx={{ color: cell[currentMode] ? color : "text.disabled" }}>{cell[currentMode] ? formatCurrency(cell[currentMode]) : "-"}</TableCell>)}
                    <TableCell align="right" sx={{ color, fontWeight: 700 }}>{formatCurrency(total)}</TableCell>
                  </TableRow>
                );
              }))
            )}
...
const NetSection = ({ records, mode }: { records: FinanceRecord[]; mode: ModeFilter }) => {
  ...
  return (
    <Card variant="outlined" sx={{ borderRadius: 3, overflow: "hidden" }}>
      <Box sx={{ px: 2.5, py: 2, bgcolor: "rgba(105,117,138,0.10)", borderBottom: "1px solid", borderColor: "divider" }}>
        <Typography variant="h6" fontWeight={700}>Net</Typography>
      </Box>
      <Box sx={{ overflowX: "auto" }}>
        <Table size="small">
          <TableHead>
          ...
          <TableBody>
            {modes.map(currentMode => {
              const total = net.reduce((sum, cell) => sum + cell[currentMode], 0);
              return (
                <TableRow key={currentMode}>
                  <TableCell><Typography variant="caption" sx={{ textTransform: "uppercase", letterSpacing: "0.12em", fontWeight: 700 }}>{currentMode}</Typography></TableCell>
                  {net.map((cell, index) => <TableCell key={index} align="right" sx={{ color: cell[currentMode] >= 0 ? "success.main" : "error.main" }}>{cell[currentMode] ? formatCurrency(cell[currentMode]) : "-"}</TableCell>)}
                  <TableCell align="right" sx={{ color: total >= 0 ? "success.main" : "error.main", fontWeight: 700 }}>{formatCurrency(total)}</TableCell>
                </TableRow>
              );
            })}
          </TableBody>
